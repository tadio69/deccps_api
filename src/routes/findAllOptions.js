  ...
  app.get('/api/options', auth, (req, res) => {
    if(req.query.nom){
      ...
      return Option.findAndCountAll({ 
        where: { 
          nom: {// nom est la propriété du modèle Option
      ...
      .then(({count, rows}) => {
        const message = `Il y a ${count} option(s) qui correspond(ent) au terme de recherche ${nom}.`
        res.json({ message, data: rows})
      })
    }else{
      Option.findAndCountAll({
        order: ['nom'],
      })
      .then(({count, rows}) => {
        const message = `La liste des ${count} options a bien été récupérée.`
        res.json({ message, data: rows})
      })
      .catch(error => {
